...
function register({ modules: { html, emmet, typescript: ts }, sourceFiles, getTsLs, htmlLs, pugLs, getCssLs, jsonLs, documentContext, vueHost, templateTsLs, getHtmlDataProviders }, getScriptContentVersion) {
    ...
    getNameCases) => {
        ...
        if ((context === null || context === void 0 ? void 0 : context.triggerKind) === vscode.CompletionTriggerKind.TriggerForIncompleteCompletions && (cache === null || cache === void 0 ? void 0 : cache.uri) === uri) {
            if ((_a = cache.lists.ts) === null || _a === void 0 ? void 0 : _a.isIncomplete) {
                cache.lists.ts = await getTsResult();
            }
            if ((_b = cache.lists.emmet) === null || _b === void 0 ? void 0 : _b.isIncomplete) {
                cache.lists.emmet = sourceFile ? await getEmmetResult(sourceFile) : undefined;
            }
            if ((_c = cache.lists.jsDoc) === null || _c === void 0 ? void 0 : _c.isIncomplete) {
                cache.lists.jsDoc = await getJsDocResult();
            }
            if ((_d = cache.lists.directiveComment) === null || _d === void 0 ? void 0 : _d.isIncomplete) {
                cache.lists.jsDoc = await getDirectiveCommentResult();
            }
            if ((_e = cache.lists.css) === null || _e === void 0 ? void 0 : _e.isIncomplete) {
                cache.lists.css = sourceFile ? await getCssResult(sourceFile) : undefined;
            }
            if ((_f = cache.lists.json) === null || _f === void 0 ? void 0 : _f.isIncomplete) {
                cache.lists.json = sourceFile ? await getJsonResult(sourceFile) : undefined;
            }
            if ((_g = cache.lists.html) === null || _g === void 0 ? void 0 : _g.isIncomplete) {
                cache.lists.html = sourceFile ? await getHtmlResult(sourceFile) : undefined;
            }
            if ((_h = cache.lists.vue) === null || _h === void 0 ? void 0 : _h.isIncomplete) {
        ...
        cache.lists.ts = await getTsResult();
        if ((_j = cache.lists.ts) === null || _j === void 0 ? void 0 : _j.items.length) {
            return combineCacheResults();
        }
        // precede html for support inline css service
        cache.lists.css = sourceFile ? await getCssResult(sourceFile) : undefined;
        if ((_k = cache.lists.css) === null || _k === void 0 ? void 0 : _k.items.length) {
            return combineCacheResults();
        }
        cache.lists.json = sourceFile ? await getJsonResult(sourceFile) : undefined;
        if ((_l = cache.lists.json) === null || _l === void 0 ? void 0 : _l.items.length) {
            return combineCacheResults();
        }
        cache.lists.html = sourceFile ? await getHtmlResult(sourceFile) : undefined;
        if ((_m = cache.lists.html) === null || _m === void 0 ? void 0 : _m.items.length) {
            return combineCacheResults();
        }
        cache.lists.vue = sourceFile ? await getVueResult(sourceFile) : undefined;
        if ((_o = cache.lists.vue) === null || _o === void 0 ? void 0 : _o.items.length) {
            return combineCacheResults();
        }
        return combineCacheResults();
        ...
        async function getTsResult() {
            ...
            for (const tsLoc of sourceFiles.toTsLocations(uri, position, position, data => !!data.capabilities.completion)) {
                ...
                if (inTemplate) {
                    const sortTexts = (_a = getTsCompletions(ts)) === null || _a === void 0 ? void 0 : _a.SortText;
                    if (sortTexts) {
        ...
        function getJsDocResult() {
            if ((context === null || context === void 0 ? void 0 : context.triggerCharacter) && !triggerCharacters.jsdoc.includes(context.triggerCharacter)) {
                return;
            }
            const result = vscode.CompletionList.create();
            for (const tsLoc of sourceFiles.toTsLocations(uri, position, position, data => !!data.capabilities.completion)) {
                if (tsLoc.type === 'source-ts' && tsLoc.lsType !== 'script')
                    continue;
                const jsDocComplete = getTsLs(tsLoc.lsType).doJsDocComplete(tsLoc.uri, tsLoc.range.start);
                if (jsDocComplete) {
                    result.items.push((0, transforms_1.transformCompletionItem)(jsDocComplete, tsRange => {
                        for (const vueLoc of sourceFiles.fromTsLocation(tsLoc.lsType, tsLoc.uri, tsRange.start, tsRange.end)) {
                            return vueLoc.range;
                        }
                    }));
        ...
        function getDirectiveCommentResult() {
            ...
            for (const tsLoc of sourceFiles.toTsLocations(uri, position, position, data => !!data.capabilities.completion)) {
                ...
                if (commentComplete) {
                    result.items = result.items.concat(commentComplete.map(item => (0, transforms_1.transformCompletionItem)(item, tsRange => {
                        for (const vueLoc of sourceFiles.fromTsLocation(tsLoc.lsType, tsLoc.uri, tsRange.start, tsRange.end)) {
                            return vueLoc.range;
                        }
                    })));
