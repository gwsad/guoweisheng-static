...
function useSfcScriptGen(lsType, vueUri, vueDoc, script, scriptSetup, scriptRanges, scriptSetupRanges, sfcTemplateCompileResult, sfcStyles, isVue2) {
    ...
    const textDocument = (0, reactivity_1.computed)(() => {
        if (lsType === 'script') {
            return vscode_languageserver_textdocument_1.TextDocument.create(`${vueUri}.${lang.value}`, shared.syntaxToLanguageId(lang.value), version++, codeGen.value.codeGen.getText());
        }
        else if (script.value || scriptSetup.value) {
            return vscode_languageserver_textdocument_1.TextDocument.create(`${vueUri}.__VLS_script.${lang.value}`, shared.syntaxToLanguageId(lang.value), version++, codeGen.value.codeGen.getText());
        }
    });
    ...
    const teleportSourceMap = (0, reactivity_1.computed)(() => {
        if (textDocument.value) {
            const sourceMap = new sourceMaps_1.TeleportSourceMap(textDocument.value);
